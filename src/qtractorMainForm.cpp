...
{
	trackInstrumentMenuReset(m_ui.trackInstrumentMenu);
}

bool qtractorMainForm::trackInstrumentMenuReset ( QMenu *pMenu ) const
{
	pMenu->clear();
//	pMenu->setStyle(scrollableMenuStyle());
	...
	// Instrument sub-menu...
	trackInstrumentMenuAdd(pMenu,
		(pTrack->pluginList())->midiManager(), sCurrentName);
	...
	for ( ; iter != iter_end; ++iter) {
		...
		QObject::connect(pBankMenu,
			SIGNAL(aboutToShow()),
			SLOT(updateTrackInstrumentBankMenu()));
	}

	return true;
}

bool qtractorMainForm::trackInstrumentMenuAdd (
...
void qtractorMainForm::updateTrackInstrumentBankMenu (void)
{
	QMenu *pBankMenu = qobject_cast<QMenu *> (sender());
	if (pBankMenu)
		trackInstrumentBankMenuReset(pBankMenu);
}

bool qtractorMainForm::trackInstrumentBankMenuReset ( QMenu *pBankMenu ) const
{
	pBankMenu->clear();
//	pBankMenu->setStyle(scrollableMenuStyle());
	...
	const int iCurrentBank = pTrack->midiBank();

	// Instrument plug-in banks sub-menu...
	if (trackInstrumentBankMenuAdd(pBankMenu,
	...
	qtractorMidiBus *pMidiBus
		= static_cast<qtractorMidiBus *> (pTrack->outputBus());
	if (pMidiBus && pMidiBus->pluginList_out()
		&& trackInstrumentBankMenuAdd(pBankMenu,
	...
	for ( ; patch_iter != patch_end; ++patch_iter) {
		...
		const QString& sBankItem = sItemMask.arg(iBank).arg(sBankName);
		QMenu *pProgMenu = pBankMenu->addMenu(icon, sBankItem);
		...
		QObject::connect(pProgMenu,
			SIGNAL(aboutToShow()),
			SLOT(updateTrackInstrumentProgMenu()));
	}
...
bool qtractorMainForm::trackInstrumentBankMenuAdd (
	...
	const QString& sInstrumentName, int iCurrentBank ) const
{
	...
	for ( ; bank_iter != bank_end; ++bank_iter) {
		...
		const QString& sBankItem = sItemMask.arg(iBank).arg(bank.name);
		QMenu *pProgMenu = pBankMenu->addMenu(icon, sBankItem);
		QAction *pAction = pProgMenu->menuAction();
		pAction->setCheckable(true);
		pAction->setChecked(iBank == iCurrentBank);
...
// Track/Instrument bank sub-menu stabilizers.
void qtractorMainForm::updateTrackInstrumentProgMenu (void)
{
	QMenu *pProgMenu = qobject_cast<QMenu *> (sender());
	if (pProgMenu)
		trackInstrumentProgMenuReset(pProgMenu);
}

bool qtractorMainForm::trackInstrumentProgMenuReset ( QMenu *pProgMenu ) const
{
	pProgMenu->clear();
	...
	const int iCurrentProg = pTrack->midiProg();

	// Instrument plugin programs sub-menu...
	if (trackInstrumentProgMenuAdd(pProgMenu,
		(pTrack->pluginList())->midiManager(),
	...
	qtractorMidiBus *pMidiBus
		= static_cast<qtractorMidiBus *> (pTrack->outputBus());
	if (pMidiBus && pMidiBus->pluginList_out()
		&& trackInstrumentProgMenuAdd(pProgMenu,
			(pMidiBus->pluginList_out())->midiManager(),
			sInstrumentName, iBank, iCurrentProg))
		return true;
	...
	for ( ; prog_iter != prog_end; ++prog_iter) {
		...
		if (iProg < 0) continue;
		const QString& sProgItem
			= sItemMask.arg(iProg + 1).arg(prog_iter.value());
		QAction *pAction = pProgMenu->addAction(icon, sProgItem);
		pAction->setCheckable(true);
...
bool qtractorMainForm::trackInstrumentProgMenuAdd (
...
void qtractorMainForm::trackInstrumentPatch (void)
{
	qtractorTrack *pTrack = m_pTracks->currentTrack();
	if (pTrack == NULL)
		return;
			...
			if (pBankMenu) {
				...
				if (pBankAction) {
					...
					while (iter.hasNext()) {
						...
						#ifdef CONFIG_DEBUG
							qDebug("qtractorMainForm::trackInstrumentPatch(%p) "
								"sInstrumentName=\"%s\" iBank=%d iProg=%d",
								pTrack, sInstrumentName.toUtf8().constData(),
								iBank, iProg);
						#endif
							// Make it an undoable command...
							pSession->execute(
								new qtractorTrackInstrumentCommand(
									pTrack, sInstrumentName, iBank, iProg));
							// Done.
							break;
						}
					}
...
QStyle *qtractorMainForm::scrollableMenuStyle (void)
{
