...
void qtractorPluginListView::addPlugin (void)
{
	...
	for (int i = 0; i < selectForm.pluginCount(); ++i) {
		...
		pPlugin	= qtractorPluginFactory::createPlugin(
			m_pPluginList,
	...
	// Let the last plugin added be the current one...
	QListWidget::setCurrentRow(pluginItem(pPlugin));
...
void qtractorPluginListView::addAudioInsertPlugin (void)
{
	...
	if (pPlugin) {
		...
		if (m_pNextItem)
			pAddInsertPluginCommand->setNextPlugin(m_pNextItem->plugin());
...
void qtractorPluginListView::addAudioAuxSendPlugin (void)
{
	...
	if (pPlugin) {
		...
		if (m_pNextItem)
			pAddAuxSendPluginCommand->setNextPlugin(m_pNextItem->plugin());
...
void qtractorPluginListView::addMidiControlPlugin (void)
{
	...
	if (pPlugin) {
		...
		if (m_pNextItem)
			pAddMidiControlPluginCommand->setNextPlugin(m_pNextItem->plugin());
...
void qtractorPluginListView::mousePressEvent ( QMouseEvent *pMouseEvent )
{
	...
	qtractorPluginListItem *pItem
		= static_cast<qtractorPluginListItem *> (QListWidget::itemAt(pos));
	if (pItem == nullptr)
		QListWidget::setCurrentItem(nullptr);
	else
