	...
	unsigned long iPeakOffset, unsigned int iPeakLength )
{
	...
	if (iPeakOffset >= m_iBuffOffset && m_iBuffOffset < iPeakEnd) {
		...
		if (iBuffEnd >= iPeakEnd) {
			::memcpy(pPeakFrames, m_pBuffer + iBuffOffset, iPeakSize);
			return iPeakLength;
		}
	...
	::memcpy(pPeakFrames, m_pBuffer, iPeakSize);
	return iPeakLength;
}
	...
	unsigned long iFrameOffset, unsigned long iFrameLength, int width )
{
	...
	qtractorAudioPeakFactory *pPeakFactory
		= qtractorAudioPeakFactory::getInstance();
	if (pPeakFactory) {
		const int m4 = int(iPeakLength >> 2) + 1;
		const int n4 = width / m4;
		if (n4 > 8) {
		#if 0
			if (m_pPeakFrames)
				delete [] m_pPeakFrames;
		...
		qDebug("DEBUG> qtarctorAudioPeak[%p]::peakFrames(%lu, %lu, %d) NEW peakPeriod=%u file=(%p, %u, %u) m4=%d n4=%d", this, iFrameOffset, iFrameLength, width, pPeakFactory->peakPeriod(), m_pPeakFile, iPeakPeriod, iChannels, m4, n4);
