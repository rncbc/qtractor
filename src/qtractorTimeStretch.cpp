...
//---------------------------------------------------------------------------
// qtractorTimeStretch::FifoBuffer - Sample frame buffer (FIFO).
//
...
// Destructor.
qtractorTimeStretch::FifoBuffer::~FifoBuffer (void)
{
	if (m_ppBuffer) {
...
void qtractorTimeStretch::FifoBuffer::setChannels ( unsigned short iChannels )
{
	if (m_iChannels == iChannels)
		return;
...
unsigned short qtractorTimeStretch::FifoBuffer::channels (void) const
{
	return m_iChannels;
}
...
unsigned int qtractorTimeStretch::FifoBuffer::readFrames (
	float **ppFrames, unsigned int iFrames, unsigned int iOffset ) const
{
	unsigned int iMaxFrames
		= (iFrames > m_iFramesInBuffer ? m_iFramesInBuffer : iFrames);
...
unsigned int qtractorTimeStretch::FifoBuffer::receiveFrames (
...
{
	if (iFrames >= m_iFramesInBuffer) {
		unsigned int iFramesInBuffer = m_iFramesInBuffer;
		m_iFramesInBuffer = 0;
		return iFramesInBuffer;
	}

	m_iFramesInBuffer -= iFrames;
	m_iFramePos += iFrames;
...
unsigned int qtractorTimeStretch::FifoBuffer::writeFrames (
	float **ppFrames, unsigned int iFrames, unsigned int iOffset )
{
...
void qtractorTimeStretch::FifoBuffer::putFrames (
	float **ppFrames, unsigned int iFrames, unsigned int iOffset )
{
	m_iFramesInBuffer += writeFrames(ppFrames, iFrames, iOffset);
}

void qtractorTimeStretch::FifoBuffer::putFrames ( unsigned int iFrames )
{
	ensureCapacity(iFrames);
	m_iFramesInBuffer += iFrames;
}
...
//
// When using this function to output samples, also remember
// to remove the/ output samples from the buffer by calling 
// the receiveFrames(iFrames) function.
float *qtractorTimeStretch::FifoBuffer::ptrBegin (
...
unsigned int qtractorTimeStretch::FifoBuffer::frames (void) const
{
	return m_iFramesInBuffer;
}


// Returns nonzero if the sample frame buffer is empty
bool qtractorTimeStretch::FifoBuffer::isEmpty (void) const
{
	return (m_iFramesInBuffer == 0);
}
